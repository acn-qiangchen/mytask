...
| DAT-6 | The merge winner is determined by `updatedAt` timestamp: the newer source wins for settings and task/session ordering. Tasks and sessions present in the losing source but absent from the winning source are union-merged in, **except** tasks recorded in either side's deletion tombstone (`deletedTaskIds`) — those remain deleted and are never resurrected across devices. |
...
| DAT-17 | On each save, tasks/sessions/interruptions older than 6 months are written to yearly archive items (`SK="ARCHIVE#YYYY"`, one item per calendar year) and pruned from the `SK="STATE"` item, keeping the STATE item under ~150 KB for long-running users.           |
| DAT-18 | Pruning happens after `mergeStates` has been applied, so cross-device sync is never disrupted by archiving.                                                                                                                                                        |
| DAT-19 | On the Reports page, if the selected date range extends beyond the 6-month cutoff, the relevant archive years are lazy-loaded from DynamoDB and merged with the current state for display only. A loading indicator is shown while historical data is fetching.    |
